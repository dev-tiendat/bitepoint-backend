...
@Injectable()
export class TPBankService extends Gateway {
    ...
    async getHistory(): Promise<Payment[]> {
        ...
        const dataSend = {
            pageNumber: 1,
            pageSize: this.config.getTransactionCountLimit,
        ...
        try {
            ...
            const transactionsWithout = filteredTransactions.map(
                transactionInfos => ({
                    ...
                    date: moment
                        .tz(
                            transactionInfos.bookingDate,
                            'YYYY-MM-DD',
                            'Asia/Ho_Chi_Minh'
                        )
                        .toDate(),
                    account_receiver: this.config.account,
                    gate: GateWayType.TP_BANK,
                })
            );
